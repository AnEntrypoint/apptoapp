...
const transformationLibrary = {
  ...
  generateJsonData: async function(transformationInstruction) {
    try {
      ...
      // Extended system prompt with additional instructions:
      // - The AI must write files by providing the complete file contents.
      // - It must continuously refactor the code to ensure it is human readable and maintain a coherent structure.
      // - Maintain a devnotes.txt file, add important notes, and clean outdated notes.
      // - Add detailed debug logging when errors occur, then re-evaluate and remove redundant ones.
      // - Self-run until confident that the changes are complete and working.
      // - For Next.js projects, call `npx pupdebug <HTTP_ADDRESS>` to log client side logs.
...
- write: params: {path: string, content: string}  (When using this operation, always provide the complete file contents, not a diff.)
- move: params: {from: string, to: string}
- delete: params: {path: string}
- rename: params: {from: string, to: string}
- createfolder: params: {path: string}
- deleteFolder: params: {path: string}
- cli: params: {command: string}

Important Directives:
1. Maintain a file named 'devnotes.txt' in the project root. All important development notes must be added to this file and outdated notes removed continuously.
2. Add detailed debug logging to help diagnose issues when errors occur, but continuously re-evaluate and remove redundant debug logs once issues are resolved.
3. Continuously refactor to make the code human readable and ensure that the structure makes sense.
4. Re-run your operations (i.e. call yourself again) until you are certain the change is complete, has been tested, and is working.
5. For Next.js projects, after applying modifications, execute 'npx pupdebug <HTTP_ADDRESS>' so that client-side logs are captured using Puppeteer.
...
[
  ...
  {"operation": "createfolder", "params": {"path": "new_folder"}},
    ...
    for (const op of operations) {
      try {
        ...
        switch (op.operation) {
          ...
          case 'createfolder':  // renamed from writeFolder to createfolder
            await this.createfolder(op.params.path);
            break;
  ...
  readSrcDirectory: async function(srcDirectory) {
    ...
    await Promise.all(files.map(async filename => {
      // Check filename to skip node_modules and hidden folders/files.
  ...
  // Renamed function: createfolder (previously writeFolder)
